...
private:
    ...
    void serve_stl(const std::string& filepath, httplib::Response &res)
    {
        ...
        if (filepath.find(".stl") != std::string::npos) mime_type = "model/stl";
        else if (filepath.find(".obj") != std::string::npos) mime_type = "model/obj";
        else if (filepath.find(".glb") != std::string::npos) mime_type = "model/gltf-binary";
        else if (filepath.find(".gltf") != std::string::npos) mime_type = "model/gltf+json";
...
public:
    ...
    allowed_extensions({".dae", ".stl", ".obj", ".glb", ".gltf", ".png", ".jpg", ".jpeg"})
    {
        ...
        server.Get(R"(/assets/([^/]+)/(.*))", [&](const httplib::Request &req, httplib::Response &res) {
